     ...
     - 带头结点：空表判断L->next==NULL
       - 按位序插入
        ...
        - 建立单链表：
          ...
          - 后插法建立单链表
    - 双链表：在单链表的基础上增加指向前驱的指针
      -  双链表的初始化（带头结点）
        - 头结点的前驱prior和后继next都设为NULL（还未存储元素，故为NULL）
        - 分配头结点内存
      - 双链表的插入
        - 后插：
           ```cpp
           // p后插入s
           s->next=p->next;
           p->next->prior=s;  // 完成将p的后继连到s上,若p有后继才执行，即p不为最后一个
           s->prior->p;
           p->next=s;
           ```
        - 删除：
           ```cpp
           // 删除p的后继结点，p为NULL或p无后继时return false
           p->next=s->next;
           s->next->prior=p;  // q非最后一个结点时执行
           delete s;
           ```
        - 双链表的遍历：一直找prior或一直找next
     - 循环链表
       - 循环单链表：最后一个结点的尾指针指向头结点
         - 
       - 循环双链表：
